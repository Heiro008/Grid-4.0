...
GPIO.setup(GPIO_C1, GPIO.IN)
GPIO.setup(GPIO_C2, GPIO.IN)
GPIO.setup(GPIO_IR, GPIO.OUT)
GPIO.output(GPIO_IR, False)
...
class range_finder(Node):
    def __init__(self):
        ...
        self.package_picked = self.create_subscription(Bool, '/package_check',self.magnet_control, 10)
        self.package_picked

        self.package_status = self.create_publisher(Bool,"/package_picked_up",10)
    ...
    def magnet_control(self,data):
        ...
        print(data.data)
        if data.data:
            GPIO.output(GPIO_IR, True)
            if GPIO.input(GPIO_C1) and GPIO.input(GPIO_C2):
                package_status_msg = Bool()
                package_status_msg.data = True
                self.package_status.publish(package_status_msg)
